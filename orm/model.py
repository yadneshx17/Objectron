        ...
        for field_name, field_obj in fields.items():
            if field_obj.primary_key:
                if pk_name is not None:
                    ...
                    raise ModelError(
                        f"[!] Model {name} has multiple primary keys defined."
                    )
                else:
                    pk_name = field_name
...
class BaseModel(metaclass=MetaClass):
    ...
    def __init__(self, **kwargs):
        ...
        try:
            for key, value in kwargs.items():
                setattr(
                    self, key, value
                )  # set the value of named attribute of an object.
        except Exception as e:
    ...
    def create_table(cls, conn) -> None:
        ...
        try:
            ...
            sql = (
                f"CREATE TABLE IF NOT EXISTS {cls.__tablename__} ({', '.join(columns)})"
            )
            conn.execute(sql)
        except Exception as e:
    ...
    def _update(self, conn) -> None:
        ...
        vals = [
            getattr(self, f.name) for f in self._fields.values() if not f.primary_key
        ]
